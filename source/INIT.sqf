...
if (isMultiplayer) then {
	...
	revive_activated = ["Revive", 1] call BIS_fnc_getParamValue;       //removed underscore from _revive_activated
	...
	if (revive_activated == 1) then {execVM "duws_revive\reviveInit.sqf"};  //removed underscore from _revive_activated

	//  Adding TcB AIS Wounding System when selected in Params--------------------------------------------------------------------------
