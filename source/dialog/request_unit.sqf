...
switch (_index) do {
    case 0: {
        if (commandpointsblu1 >= 2) then {
            ...
            _group = group player ;
            "B_Soldier_F" createUnit [_spawnpos, _group, "if (revive_activated == 2) then {[this] execvm 'ais_injury\init_ais.sqf'};", _skill, "private"] ; // Added if (revive_activated == 2) then {[this] execvm 'ais_injury\init_ais.sqf'}; between the existing ""
        }  else  {
    ...
    case 1: {
        if (commandpointsblu1 >= 3) then {
            ...
            ctrlSetText [1000, format["%1",commandpointsblu1]];
            _group = group player ;
            "B_Soldier_GL_F" createUnit [_spawnpos, _group, "if (revive_activated == 2) then {[this] execvm 'ais_injury\init_ais.sqf'};", _skill, "private"] ;//adding AIS revive to each soldier
        }  else  {
            hint "Not enough command points";
        };
    };
