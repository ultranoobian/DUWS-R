...
if(support_HC_CAS_available) then {
    _text = ["F/A-181 Fighter Support (15 CP)", 6, '_null = [player] execVM ""support\hc_cas.sqf""', 1] call menuItemFormatter;
    _offensiveItems pushBack _text;
} else {
    _text = ["F/A-181 (N/A)", 6, "", 0] call menuItemFormatter;
    _offensiveItems pushBack _text;
};

if(support_HC_CAS1_available) then {
    _text = ["Wipeout CAS Support (20 CP)", 7, '_null = [player] execVM ""support\hc_cas1.sqf""', 1] call menuItemFormatter;
    _offensiveItems pushBack _text;
} else {
    _text = ["Wipeout (N/A)", 7, "", 0] call menuItemFormatter;
    _offensiveItems pushBack _text;
};
